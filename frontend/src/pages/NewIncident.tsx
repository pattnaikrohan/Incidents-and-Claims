  ...
  const handleSubmit = async (data: any, isDraft = false) => {
    ...
    try {
      ...
      const payload = {
        ...
        corrective_actions: Array.isArray(data.corrective_actions) ? Array.from(new Set(data.corrective_actions)).map((s:any) => typeof s === 'string' ? s.replace(/Evidence preserved \(photos;\s*logs;\s*seals\)/g, "Evidence preserved (photos, logs, seals)") : s).join('; ') : (typeof data.corrective_actions === 'string' ? data.corrective_actions.replace(/Evidence preserved \(photos;\s*logs;\s*seals\)/g, "Evidence preserved (photos, logs, seals)") : data.corrective_actions),
        incident_types: Array.isArray(data.incident_types) ? Array.from(new Set(data.incident_types)).join('; ') : (data.incident_types || data.incident_type),
        claim_types: Array.isArray(data.claim_types) ? Array.from(new Set(data.claim_types)).join('; ') : data.claim_types,
        // Explicit mappings for Power Automate logical names
        cr991_incidenttypeselectallapplicableincid: Array.isArray(data.incident_types) ? Array.from(new Set(data.incident_types)).join('; ') : (data.incident_types || data.incident_type),
        cr991_immediatecorrectiveactionselectallapplicable: Array.isArray(data.corrective_actions) ? Array.from(new Set(data.corrective_actions)).map((s:any) => typeof s === 'string' ? s.replace(/Evidence preserved \(photos;\s*logs;\s*seals\)/g, "Evidence preserved (photos, logs, seals)") : s).join('; ') : (typeof data.corrective_actions === 'string' ? data.corrective_actions.replace(/Evidence preserved \(photos;\s*logs;\s*seals\)/g, "Evidence preserved (photos, logs, seals)") : data.corrective_actions),
        cr991_intenttoclaimissuedselectallapplicable: Array.isArray(data.claim_types) ? Array.from(new Set(data.claim_types)).join('; ') : data.claim_types,
        cr991_incidenttype: Array.isArray(data.incident_types) ? Array.from(new Set(data.incident_types)).join('; ') : (data.incident_types || data.incident_type),
        cr991_immediatecorrectiveaction: Array.isArray(data.corrective_actions) ? Array.from(new Set(data.corrective_actions)).map((s:any) => typeof s === 'string' ? s.replace(/Evidence preserved \(photos;\s*logs;\s*seals\)/g, "Evidence preserved (photos, logs, seals)") : s).join('; ') : (typeof data.corrective_actions === 'string' ? data.corrective_actions.replace(/Evidence preserved \(photos;\s*logs;\s*seals\)/g, "Evidence preserved (photos, logs, seals)") : data.corrective_actions),
        cr991_intenttoclaimissued: Array.isArray(data.claim_types) ? Array.from(new Set(data.claim_types)).join('; ') : data.claim_types,
        cr991_intenttoclaim: Array.isArray(data.claim_types) ? Array.from(new Set(data.claim_types)).join('; ') : data.claim_types,
        intent_to_claim: Array.isArray(data.claim_types) ? Array.from(new Set(data.claim_types)).join('; ') : data.claim_types,
